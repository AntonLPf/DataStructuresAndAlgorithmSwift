...
public struct LinkedList<Value> {
    ...
    // return a node at given index O(index)
    ...
    // insert after particular node O(1)
    @discardableResult
    ...
    private mutating func copyNodes(returningCopyOf node: Node<Value>?) -> Node<Value>? {
        ...
        while let nextNode = oldNode.next {
            if oldNode === node {
                nodeCopy = newNode
            }
            newNode!.next = Node(value: nextNode.value)
            newNode = newNode!.next
            oldNode = nextNode
        }
